...
const Character = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        const response = await axios.get(`https://site--marvbackend--s4qnmrl7fg46.code.run/character/${characterId}`)
      ...
      <div className="wrapper">
        <div className="boxCard">
          <article className='card'>
            ...
            <img src={`${data.thumbnail.path}/standard_xlarge.${data.thumbnail.extension}`} alt="comics" />
            {data.description ? (<p>
              {data.description}
            </p>) : (<p></p>)}
            {data.comics.length === 0 ? (<div className="linkComicsChar"><Link to={`/comics/${characterId}`}>
              Tout ses comics
            </Link> </div>) : (<div></div>)}
